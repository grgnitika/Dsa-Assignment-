...
// Your task is to determine the minimum time needed to build all the engines using the available 
// engineers. The engineers can either work on building an engine or split into two engineers, with 
// each engineer sharing the workload equally. Both decisions incur a time cost.
// The time cost of splitting one engineer into two engineers is given as an integer split. Note 
// that if two engineers split at the same time, they split in parallel so the cost would be split.
// Your goal is to calculate the minimum time needed to build all the engines, considering the 
// time cost of splitting engineers.
// Input: engines= [1,2,3]
// Split cost (k)=1
// Output: 4
// Example:
// Imagine you need to build engine represented by an array [1,2,3]   where ith element of an array 
// i.e a[i] represents unit time to build ith engine and the split cost is 1. Initially, there is 
...
// 2.	Again, split second engineer into two (split time :1) and assign them to build first 
...
public class MinTimeToBuildEngines {
    ...
    // Calculate the maximum time needed to build engines from start to end index
